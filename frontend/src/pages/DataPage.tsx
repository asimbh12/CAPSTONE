...
import type { BackupRecord, ImportReport, SystemReadiness } from '../types/career'
...
export function DataPage() {
  ...
  async function runReadinessChecks() {
    setBusy(true); setError(null)
    try { setReadiness(await careerApi.systemReadiness()) }
    catch (reason) { setError(reason instanceof Error ? reason.message : 'System readiness checks could not be completed') }
    finally { setBusy(false) }
  }
  ...
  return (
    <>
      ...
      <Grid container spacing={3}>
        <Grid size={12}><Card><CardContent><Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between" gap={2} alignItems={{ md: 'center' }}><Box><Typography variant="h5" mb={1}>MVP readiness</Typography><Typography color="text.secondary">Check database migrations and integrity, local storage, document checksums, AI handling policies, recoverable backups, failed workflows and audit history.</Typography></Box><Button variant="contained" startIcon={<FactCheckOutlined />} onClick={() => void runReadinessChecks()} disabled={busy}>Run readiness checks</Button></Stack>{readiness && <Box mt={3}><Alert severity={readiness.status === 'ready' ? 'success' : readiness.status === 'attention' ? 'warning' : 'error'}><Typography fontWeight={800}>{readiness.status === 'ready' ? 'MVP checks passed' : readiness.status === 'attention' ? 'MVP checks need attention' : 'MVP release is blocked'}</Typography><Typography>{readiness.passed} passed · {readiness.warnings} warnings · {readiness.failures} failures</Typography></Alert><Grid container spacing={1.5} mt={0.5}>{readiness.checks.map((check) => <Grid key={check.key} size={{ xs: 12, md: 6 }}><Card variant="outlined" sx={{ height: '100%' }}><CardContent><Stack direction="row" justifyContent="space-between" gap={1}><Typography fontWeight={800}>{check.label}</Typography><Chip size="small" label={check.status} color={check.status === 'pass' ? 'success' : check.status === 'warning' ? 'warning' : 'error'} /></Stack><Typography color="text.secondary" mt={1}>{check.message}</Typography>{check.details.map((detail) => <Typography key={detail} variant="caption" display="block" color="text.secondary" mt={0.5}>{detail}</Typography>)}</CardContent></Card></Grid>)}</Grid></Box>}</CardContent></Card></Grid>
