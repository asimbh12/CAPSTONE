  ...
  return (
    <>
      ...
      <Grid container spacing={3}>
        ...
        <Grid size={{ xs: 12, lg: 6 }}><Card sx={{ height: '100%' }}><CardContent><Typography variant="h5" mb={1}>Full local backup</Typography><Typography color="text.secondary" mb={3}>Create a consistent SQLite snapshot plus original, derived, and generated files with a checksum manifest.</Typography><Button variant="contained" startIcon={<BackupOutlined />} onClick={() => void createBackup()} disabled={busy}>Create backup</Button>{backup && <Alert severity="success" sx={{ mt: 2 }}>Backup created: {backup.filename}<br /><Button href={downloadUrl(backup.download_url)} startIcon={<DownloadOutlined />}>Download backup</Button></Alert>}</CardContent></Card></Grid>
