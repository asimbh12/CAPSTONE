  ...
  return <>
    ...
    <Card sx={{ mb: 3 }}><CardContent><Typography variant="h6" gutterBottom>Start an application</Typography>
      <Box component="form" onSubmit={event => void create(event)}><Grid container spacing={2}>
        ...
        <Grid size={12}><TextField fullWidth multiline minRows={5} label="Paste position description" helperText={file ? 'The uploaded document will be used.' : 'Paste at least 20 characters, or upload PDF, DOCX or TXT.'} value={description} disabled={Boolean(file)} onChange={event => setDescription(event.target.value)} /></Grid>
        <Grid size={{ xs: 12, md: 6 }}><Button component="label" variant="outlined" startIcon={<UploadFileOutlined />}>Choose position document<input hidden type="file" accept=".pdf,.docx,.txt" onChange={event => setFile(event.target.files?.[0] ?? null)} /></Button>{file && <Chip sx={{ ml: 1 }} label={file.name} onDelete={() => setFile(null)} />}</Grid>
        ...
        <Grid size={12}><Button type="submit" variant="contained" disabled={busy || !confirmed || !role || (!file && description.trim().length < 20)} startIcon={busy ? <CircularProgress size={18} /> : <DescriptionOutlined />}>Import and extract requirements</Button></Grid>
      </Grid></Box>
    </CardContent></Card>
    {items && items.length > 0 && <Grid container spacing={3}>
      ...
      <Grid size={{ xs: 12, lg: 9 }}>{selected && <Stack spacing={2}>
        ...
        {selected.drafts.length > 0 && <Card><CardContent><Stack direction={{ xs: 'column', md: 'row' }} justifyContent="space-between"><Typography variant="h6">Application materials</Typography><Stack direction="row" gap={1}><Button startIcon={<DownloadOutlined />} href={downloadUrl(`/applications/${selected.id}/export/docx`)}>DOCX</Button><Button startIcon={<DownloadOutlined />} href={downloadUrl(`/applications/${selected.id}/export/pdf`)}>PDF</Button></Stack></Stack><Tabs value={tab} onChange={(_, value: number) => setTab(value)} variant="scrollable">{selected.drafts.map(draft => <Tab key={draft.id} label={draftLabels[draft.draft_type]} />)}</Tabs>{selected.drafts[tab] && <TextField fullWidth multiline minRows={14} value={selected.drafts[tab].content} slotProps={{ input: { readOnly: true } }} sx={{ mt: 2 }} />}</CardContent></Card>}
      </Stack>}</Grid>
    </Grid>}
