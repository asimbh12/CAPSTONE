...
export function ProfilePage() {
  ...
  const [achievingGoal, setAchievingGoal] = useState<Goal | null>(null)
  const [achievementDate, setAchievementDate] = useState(new Date().toISOString().slice(0, 10))
  const [achievementImpact, setAchievementImpact] = useState('')
  ...
  function openAchievement(goal: Goal) {
    setAchievingGoal(goal)
    setAchievementDate(new Date().toISOString().slice(0, 10))
    setAchievementImpact(goal.description)
  }

  async function achieveGoal() {
    if (!achievingGoal) return
    setSaving(true); setError(null)
    try {
      await careerApi.achieveGoal(achievingGoal.id, {
        achieved_date: achievementDate,
        impact_summary: achievementImpact,
      })
      setGoals(current => current.filter(goal => goal.id !== achievingGoal.id))
      setFeedback('Goal marked as achieved and added to Career assets.')
      setAchievingGoal(null)
    } catch (reason) {
      setError(reason instanceof Error ? reason.message : 'Unable to mark goal as achieved')
    } finally { setSaving(false) }
  }
  ...
  return (
    <>
      ...
      <Grid container spacing={3}>
        ...
        <Grid size={{ xs: 12, lg: 7 }}><Card><CardContent><Typography variant="h5" mb={1}>Strategic goals</Typography><Typography color="text.secondary" mb={2}>Adopted goals remain user-authoritative. Mark completed goals as achieved to preserve them as career assets, or remove goals that are obsolete.</Typography><Stack spacing={1.5} mb={3}>{goals.map((goal) => <Box key={goal.id} sx={{ p: 2, border: '1px solid', borderColor: 'divider', borderRadius: 2 }}><Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" gap={2}><Box><Typography fontWeight={800}>{goal.title}</Typography><Chip sx={{ mt: 0.75 }} size="small" label={goal.horizon.replace('_', ' ')} /></Box><Stack direction="row" gap={1}><Button size="small" color="success" variant="outlined" startIcon={<TaskAltOutlined />} onClick={() => openAchievement(goal)}>Mark achieved</Button><Button size="small" color="error" startIcon={<DeleteOutlineOutlined />} onClick={() => setRemovingGoal(goal)}>Remove</Button></Stack></Stack>{goal.description && <Typography color="text.secondary" mt={0.5}>{goal.description}</Typography>}</Box>)}{goals.length === 0 && <Typography color="text.secondary">No goals yet.</Typography>}</Stack><Box component="form" onSubmit={(event) => void addGoal(event)}><Grid container spacing={1.5}><Grid size={{ xs: 12, md: 7 }}><TextField required fullWidth size="small" label="Goal title" value={goalTitle} onChange={(event) => setGoalTitle(event.target.value)} /></Grid><Grid size={{ xs: 12, md: 5 }}><TextField select fullWidth size="small" label="Horizon" value={goalHorizon} onChange={(event) => setGoalHorizon(event.target.value as Goal['horizon'])}><MenuItem value="short_term">Short term</MenuItem><MenuItem value="medium_term">Medium term</MenuItem><MenuItem value="long_term">Long term</MenuItem></TextField></Grid><Grid size={12}><TextField fullWidth size="small" label="Description" value={goalDescription} onChange={(event) => setGoalDescription(event.target.value)} /></Grid><Grid size={12}><Button type="submit" variant="outlined" startIcon={<AddOutlined />}>Add goal</Button></Grid></Grid></Box></CardContent></Card></Grid>
      </Grid>
      ...
      <Dialog open={Boolean(achievingGoal)} onClose={() => setAchievingGoal(null)} maxWidth="sm" fullWidth>
        <DialogTitle>Mark strategic goal as achieved?</DialogTitle>
        <DialogContent>
          <Stack spacing={2} mt={0.5}>
            <Typography>This will create an active career asset for <strong>{achievingGoal?.title}</strong> and remove it from the active goal trajectory. Existing readiness history will be retained.</Typography>
            <TextField required type="date" label="Achievement date" value={achievementDate} onChange={(event) => setAchievementDate(event.target.value)} slotProps={{ inputLabel: { shrink: true } }} />
            <TextField multiline minRows={3} label="Impact summary" helperText="Describe what was achieved and why it matters. You can refine this later in Career assets." value={achievementImpact} onChange={(event) => setAchievementImpact(event.target.value)} />
          </Stack>
        </DialogContent>
        <DialogActions><Button onClick={() => setAchievingGoal(null)}>Cancel</Button><Button color="success" variant="contained" disabled={saving || !achievementDate} startIcon={<TaskAltOutlined />} onClick={() => void achieveGoal()}>Add to Career assets</Button></DialogActions>
      </Dialog>
    </>
  )
}
