...
export interface DashboardAction {
  ...
  page: 'overview' | 'onboarding' | 'assets' | 'timeline' | 'opportunities' |
    'applications' | 'documents' | 'fellowships' | 'awards' | 'targets' | 'profile' | 'data'
  priority: number
  count: number
...
export interface AwardPathway {
  id: string
  name: string
  organisation: string
  award_type: 'eureka' | 'clunies_ross' | 'research_australia' |
    'prime_ministers' | 'industry' | 'custom'
  website: string
  deadline: string | null
  status: 'exploring' | 'preparing' | 'seeking_nominator' | 'ready' |
    'submitted' | 'awarded' | 'unsuccessful' | 'paused' | 'archived'
  target_id: string | null
  opportunity_id: string | null
  nominator_name: string
  nominator_status: 'not_identified' | 'candidate' | 'approached' |
    'confirmed' | 'not_required'
  dossier_status: 'not_started' | 'evidence_review' | 'drafting' | 'review' | 'complete'
  next_action: string
  notes: string
  readiness_score: number | null
  readiness_confidence: number | null
